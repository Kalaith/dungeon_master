...
const NPCCard: React.FC<NPCCardProps> = ({ npc, onSelect, selected }) => (
  ...
  >
    <div
      className={`w-12 h-12 rounded-lg flex items-center justify-center text-white font-bold text-lg ${getPortraitColor(npc.portraitClass)}`}
    >
      {npc.name.charAt(0)}
    </div>
    <div className="flex-1 min-w-0">
      ...
      <p className="text-sm text-gray-500 mb-2">
        {npc.class} • {npc.personality}
      </p>
      <div className="flex items-center gap-2">
        <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-200 ${getMoodColor(npc.mood)}`}
