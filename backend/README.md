...
Plain PHP backend for Dungeon Master using WebHatchery bearer-token auth and guest sessions.
...
Run `database/migrations/001_create_players_and_saves.sql` against the `dungeon_master` database. The migration uses `CREATE TABLE IF NOT EXISTS` and is safe to rerun for the baseline tables.
